...
from ui.components import StyledButton, StyledLabel, StyledEntry, CardFrame, StyledCombobox
...
class ImageViewerWindow:
    def __init__(self, parent, directory):
        try:
            ...
            self.window.configure(bg=AppStyles.colors['bg'])
    ...
    def setup_ui(self):
        ...
        info_frame = CardFrame(self.window, padx=10, pady=5, height=40)
        info_frame.pack(fill=tk.X, padx=10, pady=(10, 0))
        info_frame.pack_propagate(False)  # Prevent frame from shrinking
        
        self.filename_label = StyledLabel(info_frame, text="", style="subheading", anchor="w")
        ...
        sort_combo = StyledCombobox(info_frame, textvariable=self.sort_display, 
                                    values=sort_options, width=18)
        sort_combo.pack(side=tk.RIGHT, padx=(5, 10))
        sort_combo.bind("<<ComboboxSelected>>", self.on_sort_changed)
        
        StyledLabel(info_frame, text="Sort by:", style="body_small").pack(side=tk.RIGHT)
        
        # Counter labels container
        counter_frame = tk.Frame(info_frame, bg=AppStyles.colors['card'])
        counter_frame.pack(side=tk.RIGHT, padx=10)
        
        self.counter_label = StyledLabel(counter_frame, text="", style="body_small")
        self.counter_label.pack()
        
        self.subfolder_counter_label = StyledLabel(counter_frame, text="", style="body_tiny", 
                                                   fg=AppStyles.colors['text_secondary'])
        self.subfolder_counter_label.pack()
        
        # Main content area with tree and image
        content_frame = tk.Frame(self.window, bg=AppStyles.colors['bg'])
        content_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Left panel - Folder tree and jump controls
        tree_frame = CardFrame(content_frame, width=200)
        ...
        jump_control_frame = tk.Frame(tree_frame, bg=AppStyles.colors['card'])
        ...
        # Jump to - row 0
        StyledLabel(jump_control_frame, text="Jump to:", style="body_small").grid(row=0, column=0, sticky="w", padx=(0, 5))
        
        self.jump_entry = StyledEntry(jump_control_frame)
        self.jump_entry.grid(row=0, column=1, sticky="ew", padx=(0, 2))
        self.jump_entry.bind('<Return>', lambda e: self.jump_to_index())
        
        jump_btn = StyledButton(jump_control_frame, text="Go", 
                               command=self.jump_to_index,
                               style="secondary", width=3, pady=1)
        jump_btn.grid(row=0, column=2, sticky="e")
        
        # In folder - row 1
        StyledLabel(jump_control_frame, text="In folder:", style="body_tiny", 
                   fg=AppStyles.colors['text_secondary']).grid(row=1, column=0, sticky="w", padx=(0, 5), pady=(3, 0))
        
        self.subfolder_jump_entry = StyledEntry(jump_control_frame)
        ...
        subfolder_jump_btn = StyledButton(jump_control_frame, text="Go", 
                                         command=self.jump_to_subfolder_index,
                                         style="secondary", width=3, pady=1)
        subfolder_jump_btn.grid(row=1, column=2, sticky="e", pady=(3, 0))
        
        # Separator
        tk.Frame(tree_frame, bg=AppStyles.colors['border'], height=1).pack(fill=tk.X, pady=(5, 5))
        
        # Folder tree
        StyledLabel(tree_frame, text="Folders", style="subheading").pack(pady=(0, 5))
        ...
        nav_frame = CardFrame(self.window, padx=10, pady=10, height=50)
        nav_frame.pack(fill=tk.X, padx=10, pady=(0, 10))
        nav_frame.pack_propagate(False)  # Prevent frame from shrinking
        
        self.prev_btn = StyledButton(nav_frame, text="← Previous", 
                                     command=self.previous_image,
                                     style="primary", width=12, pady=8)
        self.prev_btn.pack(side=tk.LEFT, padx=5)
        
        self.next_btn = StyledButton(nav_frame, text="Next →", 
                                     command=self.next_image,
                                     style="primary", width=12, pady=8)
        self.next_btn.pack(side=tk.LEFT, padx=5)
        
        # Action buttons
        open_location_btn = StyledButton(nav_frame, text="Explorer", 
                                        command=self.open_in_explorer,
                                        style="secondary", width=10, pady=5)
        open_location_btn.pack(side=tk.LEFT, padx=5)
        self._create_tooltip(open_location_btn, "Open file location in Windows Explorer")
        
        open_viewer_btn = StyledButton(nav_frame, text="Viewer", 
        ...
        open_paint_btn = StyledButton(nav_frame, text="Paint", 
